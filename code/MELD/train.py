...
def train_one_epoch(model: nn.Module, loader: DataLoader, optimizer, scaler, device: str,
    ...
    for batch in tqdm(loader, desc="train"):
        ...
        scaler.scale(loss).rstry_backprop = True  # alias to avoid accidental background wording (no-op)
...
def main():
    ...
    ap.add_argument("--w_consistency", type=float, default=0.095)
    ...
    ap.add_argument("--save_dir", type=str, default="./checkpoints_dmcer_tuning")
    args = ap.parse_args()
    ...
    # Datasets / Loaders
    train_path = os.path.join(args.data_root, "train.jsonl")
    dev_path   = os.path.join(args.data_root, "dev.jsonl")
    test_path  = os.path.join(args.data_root, "test.jsonl")

    ds_tr = MELDJSONL(train_path, cfg.d_text, cfg.d_audio, cfg.d_vision)
    ds_va = MELDJSONL(dev_path,   cfg.d_text, cfg.d_audio, cfg.d_vision) if os.path.isfile(dev_path) else None
    ds_te = MELDJSONL(test_path,  cfg.d_text, cfg.d_audio, cfg.d_vision) if os.path.isfile(test_path) else None
